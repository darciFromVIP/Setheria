...
namespace FoW
{
    ...
    public class FogOfWarTeam : MonoBehaviour
    {
		...
		public byte[] GetCurrentFogValues(byte[] values)
		{
			GetCurrentFogValues(values);
			return values;
		}
		...
		public byte[] GetTotalFogValues(byte[] values)
		{
			GetTotalFogValues(values);
			return values;
		}
        ...
        /// <summary>
        /// Returns the current fog amount at a particular world position. 0 is fully unfogged and 255 if fully fogged.
        /// </summary>
        /// <param name="position"></param>
        /// <returns></returns>
        public byte GetCurrentFogValue(Vector3 position)
        {
            Vector2Int mappos = WorldPositionToFogPosition(position);
            mappos.x = Mathf.Clamp(mappos.x, 0, mapResolution.x - 1);
            mappos.y = Mathf.Clamp(mappos.y, 0, mapResolution.y - 1);
            return _fogValuesCurrent[mappos.y * mapResolution.x + mappos.x];
        }
        ...
        {
            foreach (FogOfWarUnit unit in FogOfWarUnit.registeredUnits)
            {
